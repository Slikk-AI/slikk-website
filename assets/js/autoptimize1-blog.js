...
document.addEventListener("DOMContentLoaded", function () {
    jQuery(document).ready(function ($) {
        ...
        function remoji_fetch_reaction_panel(that) {
            $.get(remoji.show_reaction_panel_url, function (res) {
                if (res._res !== "ok") {
                    $(".remoji_error_bar[data-remoji-id=" + remoji_id + "][data-remoji-type=" + remoji_type + "]")
            ...
            $.ajax({
                ...
                success: function (res) {
                    if (res._res !== "ok") {
                        ...
                        $(".remoji_error_bar[data-remoji-id=" + this_remoji_id + "][data-remoji-type=" + this_remoji_type + "]")
                            .css("display", "inline-block")
                            .show()
                            .fadeOut(2000);
                    } else {
                        ...
                        if ($(this_selector).length > 0) {
                            ...
                            var new_ele = $("<div />")
                                .addClass("remoji_container")
                                .attr("data-remoji-id", this_remoji_id)
                                .attr("data-remoji-type", this_remoji_type)
                                .attr("data-remoji-name", this_remoji_name)

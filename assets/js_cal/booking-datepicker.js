...
(function(factory){
	...
	* ================== */
	...
	$('#from-date').datepicker({format: "dd-mm-yyyy"}).on('changeDate', function(x){
	...
	$('#multi').datepicker({format: "dd-mm-yyyy"}).on('changeDate', function(multi){
		var dates = ''
		multi['dates'].forEach(date => {
			date = date.toString()
			if(dates == ''){
				dates += date.split(' ').slice(1,4).join(' ')
			}
			else{
				dates += ', ' + date.split(' ').slice(1,4).join(' ')
			}

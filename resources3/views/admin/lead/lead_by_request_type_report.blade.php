...
<div class="xp-breadcrumbbar text-center">
    <h4 class="page-title">Lead By Request Type</h4>  
      <ol class="breadcrumb">
        <li class="breadcrumb-item"><a href="#">Marketing Sales Report</a></li>
        <li class="breadcrumb-item active" aria-current="page">Lead By Request Type</li>
      </ol>                
</div>
    ...
    <div class="row">
        ...
        <div class="col-md-12 col-lg-12 col-xl-12">
            <div class="card m-b-30">
                <div class="card-body">
					<form method="post" action="{{url('lead-by-request-type-report')}}">
						@csrf
						<div class="form-row">
							...
							$Employee = 1;
								if($LoginSessionValue->role_id==1)
									$Employee = 0;
								if($LoginSessionValue->role_id == 2)
									$Employee = 0;
							@endphp
							<div class="form-group col-md-4">
								<input type="text" class="form-control search_agent" name="EmployeeSearch" {{$Employee==1?'readonly':''}}  value="{{$EmployeeSearch}}" id="EmployeeSearch" placeholder="Search Employee">
            ...
            <div class="card m-b-30">
				<div class="row">
					<div class="col-md-12">
						...
						<div class="card-body">
							 <div id="xp-c3-pie-request-type" style="height:320px;"></div>
						</div>
					</div>
                ...
                <div class="card-body">
					...
					<ul class="nav nav-tabs nav-justified mb-3" id="defaultTabJustified" role="tablist">
						...
						@endif  
						    <li class="nav-item">
								<a class="nav-link {{$active}} tabATAg" id="{{$Id}}-tab-justified" data-toggle="tab" href="#{{$Id}}-justified" role="tab" aria-controls="{{$Id}}" aria-selected="false"><span class="lead_status">{{$TabMenuData['name']}}</span> <span class="{{$Id}}-count {{$TabMenuData['color']}}">{{'('.$TabMenuData['count'].')'}}</span></a>
							</li>
						@endforeach
...
@if(Helper::allow_permission('view-lead-by-request-type-report')==1)
<script>
$(function() {
	...
	getRequestTypeLeadCount(); // Lead By Request Type
	...
	var SearchValue = $(this).find('.lead_status').text();
...
function getRequestTypeLeadCount(){
	...
	type:"POST",
    async: false,
	url:"{{url('getRequestTypeLeadCount')}}",
	data:{
		"_token": "{{ csrf_token() }}",
		...
		success:function(response){
			...
			$('#request_type_color_code').val(JSON.stringify(res.request_type_color_code));
			$('#request_type_column_values').val(JSON.stringify(res.request_type_column_values));
			var ColorCode = res.request_type_color_code;
			var ColumnName = res.request_type_column_values;
			xpPieChart('xp-c3-pie-request-type', ColorCode,ColumnName);
		},
	});
	//alert(Dateselection);
}
...
function getListDatatable(SearchValue){
	...
	$('#datatable-buttons').DataTable({
        ...
        "ajax": {
			'type': 'POST',
            "url": "{{url('lead_list_by_request_type')}}",
			//"dataSrc": "",
            'data': {
			   "_token": "{{ csrf_token() }}",
			   action: 'List',

				...
				<div class="row">
					<div class="col-md-6">
						<div class="card-header bg-white">
							<h5 class="card-title text-black">Lead List {{($AgentId)?'(Agent: '.$AgentDetails[0]->user_name.')':''}} </h5>
						</div>
					</div>
...
@endsection
@section('script')
@if(Helper::allow_permission('type-of-lead')==1)
<script>
$(function() {
	$.ajaxSetup({
		headers: {
...
function getLeadsByCategory(){
	...
	$.ajax({
	type:"POST",
	url:"{{url('getLeadStatusCountByCategory')}}",
	data:{
		"_token": "{{ csrf_token() }}",
		CategoryId:Category,
		AgentId:AgentId,

...
class BlogController extends Controller
{
	...
	*********************************************************/
	...
	public function blog_list(request $response) {
		...
		if ($Result->count() > 0) {
			foreach ($Result as $row) {
				...
				$view_url = url('blog/'.$row->slug.'/'.$row->url);
				//$view_url = '';
				$report_sequence_url = url('arrange_blog_section_sequence/'.$Id);
				$row_data['action'] = '<a href="'.$edit_url.'" class="EditData "  title="Edit"><i class="fa fa-edit"></i></a> - <a target="new" href="'.$view_url.'"class="ViewData " title="View"><i class="fa fa-eye"></i></a> - <a target="new" href="'.$report_sequence_url.'"class="ViewData " title="Arrange Report Section Sequence"><i class="fa fa-sort"></i></a> - <a href="javascript:void(0);" class="DeleteRoom " onclick="return deleteData('.$Id.');" title="Delete"><i class="fa fa-trash"></i></a>';
				$data[] = $row_data;
			}
	...
	public function page_list(request $response) {
		...
		if ($Result->count() > 0) {
			foreach ($Result as $row) {
				...
				$row_data['action'] = '<a href="'.$edit_url.'" class="EditData "  title="Edit"><i class="fa fa-edit"></i></a> - <a href="'.$view_url.'" class="EditData "  title="Edit"><i class="fa fa-eye"></i></a> - <a target="new" href="'.$page_sequence_url.'"class="ViewData " title="Arrange Page Section Sequence"><i class="fa fa-sort"></i></a> - <a href="javascript:void(0);" class="DeleteRoom " onclick="return deleteData('.$Id.');" title="Delete"><i class="fa fa-trash"></i></a> - <a href="'.$add_content_url.'" class="EditData "  title="Edit">Add Element</a> - <a href="'.$element_list_url.'" class="EditData "  title="Edit">Element List</a>';
				$data[] = $row_data;
			}
	...
	function destroy($id)
	{
		...
		$delete=DB::table('mr_blog')->where('id',$id)->delete();
	...
	function destroypage($id)
	{
		...
		$delete=DB::table('mr_page')->where('id',$id)->delete();
		echo 'Record deleted successfully!';
	}
	...
	function post_blog(request $response){
		$data['title'] =  'Blog';
		$data['category_slug']=$response->category_slug;
		$ReportData = $this->Common->getList('mr_categories', 'slug', $data['category_slug']);
		$data['report_data'] = $ReportData[0];
		$data['BlogDetails'] = $this->Blog->getBlogDetails_preview($response->slug);
		if(count($data['BlogDetails'])==0){
            return redirect()->to('blogs');
		...
		$data['reportSectionDetails'] =  $this->Blog->getData('mr_blog_sections', $WhrArr, 'section_sequence');
		return view('frontnew.blog-detail',$data);
    }
	function post_blog_old(request $response){
		$data['title'] =  'Blog';
		$data['category_slug']=$response->category_slug;
		$ReportData = $this->Common->getList('mr_categories', 'slug', $data['category_slug']);
		$data['report_data'] = $ReportData[0];
	...
	function blogs(){
		...
		return view('frontnew.blog',$data);
    }
	function old_blog(){
		$url = \Request::segment(1);
        $ReportData = $this->Common->getList('mr_page', 'url', $url);
	...
	function blogs_cat(request $response){
        ...
        $ReportData = $this->Common->getList('mr_categories', 'slug', $url);
		$data['report_data'] = $ReportData[0];
		$data['meta_title'] = $data['report_data']->meta_title;
		$data['meta_keywords'] = $data['report_data']->meta_description;
		$data['meta_description'] = $data['report_data']->meta_keywords;
		$data['bottom_content'] = $data['report_data']->description;
		$data['title'] =  'Blogs2';
		//$FilterCategory = $response->category_slug;

		//echo '<pre>'; print_r($url); exit;
		//$FilterCategory = 'abc';
		// print_r($cat_id);
		// exit();
	// $FilterCategory_data= $data['category_ids'] = $this->Blog->getList_cat($url);
	// $FilterCategory = $FilterCategory_data[0]->category_id;
	// print_r($FilterCategory);
	// 	exit();
		$data['AllBlogs'] = $this->Blog->getAllBlog_cat(12, $cat_id);
		$data['ResultPopularBlogs'] = $this->Blog->getPopularBlogs(4);
		$data['category_data'] = $this->Common->getList('mr_categories', 'status', 1);
	...
	function submit_page(request $response){
		...
		$dataArr= array(
			...
			'banner_title'=>$response->BannerTitle,
			'banner_description'=>$response->BannerDescription,
			'status'=>$ReportStatus,
		);
		if ($response->hasFile('BannerBackgroundImage')) {
			$image = $response->file('BannerBackgroundImage');
			$name = $image->getClientOriginalName();
			$destinationPath = 'assets/page';
			$image->move($destinationPath, $name);
			$dataArr['banner_background_img']=$name;
			if($response->HiddenBannerBackgroundImage!=''){
				$filename = 'assets/page/'.$response->HiddenBannerBackgroundImage;
				if (file_exists($filename)){
				   @unlink($filename);
				}
			}
		}
		if ($response->hasFile('BannerImage')) {
			$image = $response->file('BannerImage');
			$name = $image->getClientOriginalName();
			$destinationPath = 'assets/page';
			$image->move($destinationPath, $name);
			$dataArr['banner_image']=$name;
			if($response->HiddenBannerImage!=''){
				$filename = 'assets/page/'.$response->HiddenBannerImage;
				if (file_exists($filename)){
				   @unlink($filename);
				}
			}
	...
	public function  videos_new(request $response){
		$API_key = 'AIzaSyBg_5rw3LlRuEIUg-3CzdSBSRWs4T8vK9g';
		$channelID = 'UCpcQTb5HDvm4Cys12OLuFCw';
		$maxResult = 50;
	
		$apiError = 'Video not Found';
		try{
			$apiData = @file_get_contents('https://www.googleapis.com/youtube/v3/search?part=snippet&channelId='.$channelID.'&maxResults='.$maxResult.'&key='.$API_key.''); 
		...
		return view('frontnew/video',compact('videoList'));
	}
	public function video_old(request $response){
		$API_key = 'AIzaSyBg_5rw3LlRuEIUg-3CzdSBSRWs4T8vK9g';
		$channelID = 'UCpcQTb5HDvm4Cys12OLuFCw';
		$maxResult = 50;
		...
		return view('frontend/pages/video_new',compact('videoList'));
	}

// article code 
	public function  support_articles(request $response){
		return view('frontend/pages/support_articles');
	}
	public function  help_new(request $response){
		return view('frontend/pages/help_new');
	}



//  video page code end here 
	...
	public function blogdetaildesign(){
		return view('frontend/pages/blog-detail-test');
	}
	public function blogtest(request $response){
		$data['title'] =  'Blog';
		$data['category_slug']=$response->category_slug;
		$ReportData = $this->Common->getList('mr_categories', 'slug', $data['category_slug']);
		$data['report_data'] = $ReportData[0];
		...
		$data['reportSectionDetails'] =  $this->Blog->getData('mr_blog_sections', $WhrArr, 'section_sequence');
		return view('frontend/pages/blog-testt',$data);
	}
	// video page code here 

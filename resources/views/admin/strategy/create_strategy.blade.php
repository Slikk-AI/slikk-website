					...
					@csrf
						<div class="form-row">
							<div class="form-group col-md-9">
							  ...
							  <input type="text" class="form-control" name="PlanTitle" id="PlanTitle" placeholder="Plan Title" required value="{{($strategy_data)?$strategy_data->plan_title:''}}">
							  <div class="invalid-data invalid-PlanTitle"></div>
							</div>
							<div class="form-group col-md-3">
							  <label for="Price">Price<span class="text-red">*</span></label>
							  <input type="text" class="form-control" name="Price" id="Price" required placeholder="Plan Price" value="{{($strategy_data)?$strategy_data->price:''}}">
...
<script src="{{ admin_url.'assets/js/init/form-editor-init.js' }}"></script>
 ...
 <script src="{{ admin_url.'assets/js/chart_plugin.js' }}"></script>
<script src="{{ admin_url.'assets/js/custom_chart.js' }}"></script>
<script>
...
function addMoreContentSectionData(SectionType){
	...
	if(SectionType=='Content Section'){
		...
		$clone.find('.ReportHiddenPDF').attr('id','ReportHiddenImg_'+Counter).attr('name','ReportHiddenPDF['+Counter+']');
...
$(document).ready(function(e) {
	if($("#xp-toc-tinymce").length > 0) {
        tinymce.init({
            selector: "textarea#xp-toc-tinymce",
            theme: "modern",
            height:320,
            plugins: [
                "advlist autolink link image lists charmap print preview hr anchor pagebreak spellchecker",

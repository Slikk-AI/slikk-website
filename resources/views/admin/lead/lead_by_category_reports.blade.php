...
<div class="xp-breadcrumbbar text-center">
    <h4 class="page-title">Lead By Category</h4>  
      <ol class="breadcrumb">
        <li class="breadcrumb-item"><a href="#">Marketing Sales Report</a></li>
        <li class="breadcrumb-item active" aria-current="page">Lead By Category</li>
      </ol>                
</div>
    ...
    <div class="row">
        ...
        <div class="col-md-12 col-lg-12 col-xl-12">
            <div class="card m-b-30">
                <div class="card-body">
					<form method="post" action="{{url('lead-by-category-reports')}}">
						@csrf
						<div class="form-row">
            ...
            <div class="card m-b-30">
				<div class="row">
					<div class="col-md-12">
						...
						<div class="card-body">
							 ...
							 <input type="hidden" id="CatVersesRegionLabel" >
							 <input type="hidden" id="CatVersesRegionCount" >
						</div>
					</div>
                ...
                <div class="card-body">
					...
					<ul class="nav nav-tabs nav-justified mb-3" id="defaultTabJustified" role="tablist">
						...
						@endif  
						    <li class="nav-item">
								<a class="nav-link {{$active}} tabATAg" id="{{$Id}}-tab-justified" data-toggle="tab" href="#{{$Id}}-justified" role="tab" aria-controls="{{$Id}}" aria-selected="false"><input type="hidden" class="grid_value" value="{{$TabMenuData['id']}}"><span class="lead_status">{{$TabMenuData['name']}}</span> <span class="{{$Id}}-count {{$TabMenuData['color']}}">{{'('.$TabMenuData['count'].')'}}</span></a>
							</li>
						@endforeach
...
@if(Helper::allow_permission('view-lead-by-category-report')==1)
<script>
$(function() {
	...
	getDateWiseCategoryVersesRegionLeadCount(); // Lead By category
	...
	var SearchValue = $(this).find('.grid_value').val();
...
function getFilterData(){
	var SearchValue = $('#SearchValue').val();
	//getListDatatable(SearchValue);
	getDateWiseCategoryVersesRegionLeadCount(); // Lead By Request Type
}
function getFilterCancel(){
	window.location="{{url('lead-by-status-reports')}}";
}
function getDateWiseCategoryVersesRegionLeadCount(){
	...
	type:"POST",
    async: false,
	url:"{{url('getCategoryVersesRegionLeadCount')}}",
	data:{
		"_token": "{{ csrf_token() }}",
		...
		success:function(response){
			...
			$('#CatVersesRegionLabel').val(JSON.stringify(res.x_label));
			$('#CatVersesRegionCount').val(JSON.stringify(res.column_values));
			xpBarChart();
		},
	});
	//alert(Dateselection);

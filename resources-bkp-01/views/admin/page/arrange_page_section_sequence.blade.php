    ...
    <div class="row">
        ...
        <div class="col-lg-12">
            <div class="card m-b-30">
				@if(Helper::allow_permission('arrange-report-section')==1)
                <div class="card-header bg-white">
                    <h5 class="card-title text-black">{{$title}}</h5>
                </div>
                <div class="card-body">
                    <form method="post" action="{{url('submit_page_section_sequence')}}" enctype="multipart/form-data">
					@csrf
						<div class="form-row">
						<label>Page Title : {{$PageDetails[0]->title}}</label>
						</div>
						<div class="form-row">
							<div class="dd nestable col-md-12" id="nestable">
								<ol class="dd-list">
								@php
								$ArrElement = Helper::element_type();
								@endphp
								  @foreach($PageSectionDetails as $SectionData)
								  <li class="dd-item" data-id="1" data-name="Home" data-slug="home-slug-1" data-new="0" data-deleted="0">
									<div class="dd-handle">Section {{$SectionData->sort}} ({{$ArrElement[$SectionData->element_id]}}): {{$SectionData->title}} </div>
									<input type="hidden" name="SectionSequence[]" value="{{$SectionData->id}}">
								  </li>
									@endforeach
								</ol>
							  </div>
					 ...
					 <input type="hidden" name="PageId" id="PageId" value="{{$PageId}}">
					  ...
					  <a href="{{url('page_listing')}}" class="btn btn-secondary">Back</a>
                    </form>
                </div>
				@else
					<div class="card-body">
						<label class="label label-danger">You don't have permission to Arrange Report Section Sequence</label>
					</div>
				@endif
            </div>
        </div>

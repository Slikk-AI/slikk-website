            ...
            <div class="card m-b-30">
                ...
                <div class="card-body">
					...
					<ul class="nav nav-tabs nav-justified mb-3" id="defaultTabJustified" role="tablist">
						...
						@endif  
						    <li class="nav-item">
								<a class="nav-link {{$active}} tabATAg" id="{{$Id}}-tab-justified" data-toggle="tab" href="#{{$Id}}-justified" role="tab" aria-controls="{{$Id}}" aria-selected="false"><span class="lead_status">{{$TabMenuData['name']}}</span> <span class="{{$Id}}-count {{$TabMenuData['colorClass']}}">{{'('.$TabMenuData['count'].')'}}</span></a>
							</li>
						@endforeach
							<li class="nav-item">
								<a class="nav-link  tabATAg" id="all-tab-justified" data-toggle="tab" href="#all-justified" role="tab" aria-controls="all" aria-selected="false"><span class="lead_status">All</span> <span class="all-count badge badge-primary">{{'('.$AllCount.')'}}</span></a>
							</li>
...
@endsection
@section('script')
@if(Helper::allow_permission('lead-by-status')==1)
<script>
$(function() {
	$.ajaxSetup({
		headers: {
...
function getLeadsByCategory(){
	...
	$.ajax({
	type:"POST",
	url:"{{url('getLeadTypeCountByCategory')}}",
	data:{
		"_token": "{{ csrf_token() }}",
		CategoryId:Category,
		},
